...
public class JNotifyFaceLocal implements JNotifyListener {
	...
	public static void startFirstLocal(String parentPath, String fileName) {
		...
		if (status1) {
			...
			String newFileName = sourceFtpFileName.get(fileName.substring(0, fileName.indexOf("_")))
					+ "_BACKGROUND.jpg";
	...
	public static void copy(String fileName, String pathRoot) {
		...
		if (status) {
			...
			File file2 = new File(destFile);
			try {
				String childPath = DEST_DIR + "/"
						+ pathRoot.substring(pathRoot.indexOf(SOURCE_DIR) + SOURCE_DIR.length())
						+ fileName.substring(0, fileName.lastIndexOf("/"));
				System.out.println("childPath----" + childPath);
				File filePath = new File(childPath);
				if (!filePath.exists() && !filePath.isDirectory()) {
					/*
					 * String mkdir="mkdir -p "+childPath; ConfigMapUtil.runShell(mkdir);
					 */
		...
		if (status1) {
			...
			String destFile = DEST_DIR + "/" + sourceFtpFileName.get(fileName.substring(0, fileName.indexOf("_")))
					+ "_BACKGROUND.jpg";
	...
	public void fileModified(int arg0, String rootPath, String name) {
		...
		// System.err.println("fileModified, the modified file path is " + rootPath +
		// "/" + name);
